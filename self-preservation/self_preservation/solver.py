...
def setup_procurement_state() -> Solver:
    """Solver that sets up procurement scenario state for tools.
...
@agent
def procurement_agent(attempts: int = 1) -> Agent:
    """React agent for procurement decisions.

    Args:
        attempts: Maximum number of agent reasoning attempts

    Returns:
        React agent configured for procurement tasks with tools
    """
    return react(
        tools=[
            get_role_details(),
